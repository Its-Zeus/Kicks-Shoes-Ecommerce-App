...
import filterApis from '@/app/_utils/filterApis';
import productApis from '@/app/_utils/productApis';
import Spacing from '@/app/_constants/Spacing';
import Filters from '@/app/newdrops/_components/Filters';
import ProductList from '@/app/newdrops/_components/ProductList';
...
export default function Categories( {params} : Params) {
    const [colorFilters, setColorFilters] = useState([])
    const [sizeFilters, setSizeFilters] = useState([])
    const [categoryFilters, setCategoryFilters] : [string [], any] = useState([])
    ...
    const [FiltersApplied , setFiltersApplied] = useState('TRENDING')
    ...
    useEffect (() => {
        getFilters_()
        setCategoryFilters([params.slug]);
    }, [])
    useEffect(() => {
        ...
        <div className={`${Spacing} py-10`}>
            <div className='hidden lg:flex justify-between px-2 py-7'>
                ...
                <div className='relative'>
                    ...
                    {
                        FilterPopup &&
                        <ul ref={popupRef} className='py-3 px-4 text-left font-semibold text-lgfont-semibold uppercase bg-fawhite pt-6 -mt-4 rounded-b-xl absolute z-20 left-0 right-0'>
            ...
            <div className='lg:hidden pb-5'>
                <div className='flex gap-5'>  
                    ...
                    <div className='relative'>
                        ...
                        {
                            FilterPopup &&
                            <ul ref={popupRef} className='py-3 px-4 text-left font-semibold text-lgfont-semibold uppercase bg-white pt-6 -mt-4 rounded-b-xl absolute z-20 left-0 right-0'>
                                {/* <li><button className='uppercase mb-1' onClick={(e) => {setFiltersApplied(e.currentTarget.innerText); setFilterPopup(!FilterPopup)}}>Trending</button></li> */}
                                <li><button className='uppercase mb-1' onClick={(e) => {setFiltersApplied(e.currentTarget.innerText); setFilterPopup(!FilterPopup)}}>Newest</button></li>
                                <li><button className='uppercase mb-1' onClick={(e) => {setFiltersApplied(e.currentTarget.innerText); setFilterPopup(!FilterPopup)}}>Price Asc</button></li>
                                <li><button className='uppercase mb-1' onClick={(e) => {setFiltersApplied(e.currentTarget.innerText); setFilterPopup(!FilterPopup)}}>Price Desc</button></li>
